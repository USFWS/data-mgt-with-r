...
But if you want to be the star student, or hate yourself later, a better approach is to provide thoughtful comments throughout the code that informs the reader, *succintly*, of your intentions. This isn't play-by-play. We don't need to rehash "what" we're doing with the code ("loading in my data"), or "how" the code is doing it ("looping over individuals"), but rather "why" we wrote any particular piece of code. What is that code intending to accomplish?
...
.center[![:scale 50%](images/function.jpg)]
...
.center[![:scale 50%](images/false.jpg)]
...
library(DT)
rio_file_types <- rio::import("~/FWS_Projects/Requests_for_assistance/data-mgt-with-r/docs/adam/data/rio_formats.csv")
...
---
# Data processing to us

- Reshaping
- Maniputing, transforming, cleaning
- Profiling
- Assertions
...
???
First, let's talk a bit about tabular data.

We can present it in two distinct forms: wide and long.

In 'wide' format, each sample/individual get a row, and each variable of interest occupies a column. This can be a useful format for recording data, and often for analyzing data.

In 'long' format, that 'wide' data is pivoted such that all recorded values for that sample/individual occupy a column, and there's an associated column providing the variable "context" for that value. This can be a useful format for summarizing and visualizing data.
...
.pull-left[
- [`tidyr`](https://tidyr.tidyverse.org/articles/pivot.html) (example)
...
- Not "clean", but "wide" data organized such that:
...
```{r, echo = FALSE, child="rmd/mammal_report.rmd"}
```
]]  

---
# QA/QC with assertive programming

- Test explicit assumptions/expectations
- Play QA/QC offense, not defense
...
mammals %>% assertr::verify(weight < 100)

# Alternative, giving weight range
mammals %>% assertr::assert(assertr::within_bounds(15, 100), weight)

# Confirm expected format of sex variable
mammals %>% assertr::assert(assertr::in_set(c("M", "F")))
```

???
Data interrogation and data profiling examples we just covered are a nice, graphical way to test your assumptions and expectations about your data. But they're a bit ad hoc. If we're familiar with the data we're using, or what we expect it to look like up front, then we can be more explicit and assertive so that you can detect and correct any deviations from that expectation as soon as possible in your workflow. That is, we can be on offense in our data QA/QC, rather than playing defense.

The `assertr` package gives us a suite of functions designed to verify our assumptions about the data as early as possible.

A few simple examples. So many more possibilities --> vignette

---
# Exploratory data analysis

- More thoughtful and intentional than data profiling
...
???
Once we're past the data profiling and QA/QC, we can start looking to better understand our data with exploratory data analysis.

---
# Visualization framework

- Need flexible but consistent grammar for visualization
...
There are alternative visualization tools in R, but we appreciate the flexibility and intuitiveness of ggplot, and how quickly you can go from data to publication quality figures. We could spend a solid week on ggplot, so we'll restrict the demonstration to a quick example of the building of a plot using the layers, as well as show you a few figures created by ggplot to illustrate the range of possibilities. Couple that with the relative ease of turning ggplot-based figures into animations and interactive graphics

---

```{r, child = "rmd/sav_flipbook.Rmd"}
```
...
We can't reasonably spend much time here because there's simply too much material. So we'll focus on a few good resources to lead you down the right code-based path in R for whatever analysis you're looking for. The good news is that, if it can be done, there's likely an R resource available.
...
- very focused to emphasize packages 
